...
function test_gap_column_generation_converges()
    @testset "[gap] column generation converges (2 machines, 7 tasks)" begin
        ...
        # CG must converge to a finite dual bound
        @test !isnothing(output.incumbent_dual_bound)
        @test isfinite(output.incumbent_dual_bound)

        # Dual bound must be non-negative (all costs ≥ 1)
        @test output.incumbent_dual_bound >= 0.0

        # LP relaxation objective must be finite
        @test !isnothing(output.master_lp_obj)
        @test isfinite(output.master_lp_obj)

        # At LP optimality: dual bound ≤ primal LP objective (weak duality)
        @test output.incumbent_dual_bound <= output.master_lp_obj + 1e-4
    end
end
